...
- **Client request headers now reach upstream on the raw, typed and responses-WS faces (behaviour change, deliberate)**: the gateway no longer drops all client headers on the HTTP faces. Custom and session-style headers (for example `X-Opencode-Session`, `X-Claude-Code-Session-Id`, `Session-Id`), plus client-reported protocol headers such as `Anthropic-Version` and `OpenAI-Beta`, are relayed verbatim — a client's own protocol declaration may override the SDK/gateway default. Previously the raw and typed HTTP paths forwarded nothing but the endpoint `Content-Type` and the account credential, while the WebSocket handshake path relayed everything its own 10-item list permitted. The raw, typed and responses-WS faces now share one mechanism (a single deny list); the codex WebSocket face is deliberately the opposite — it forwards no client header at all (see the entry below).
...
- **Client-IP and forwarding-path headers no longer reach upstream (behaviour change)**: `X-Forwarded-For`, `X-Real-IP`, `CF-Connecting-IP`, `True-Client-IP`, `Forwarded`, `X-Forwarded-Host`, `X-Forwarded-Proto`, `X-Forwarded-Port` and `Via` are now stripped on every face. These describe the client's own connection — who the peer was, and which host, scheme and intermediaries it came through — while upstream sees the gateway as its peer and the gateway rewrites the target URL. The gateway also does not append its own observation the way a conventional reverse proxy does, so relaying them passed a client-controlled value upstream in a shape that looks proxy-generated. This is deliberately **not** a security boundary: the list is a deny list, so other keys carrying the same kind of information (for example `X-Client-IP`, or any custom header) still reach upstream, and only the well-known carriers are removed.
- **`Expect` and `X-Request-Id` no longer reach upstream**: `Expect: 100-continue` is a negotiation about the client's connection to the gateway, not about the gateway's connection upstream, and relaying it made every request to an upstream that never sends an interim `100 Continue` pay the transport's `ExpectContinueTimeout` (measured: 1 ms without it, 1.001 s with it — a client-injectable delay). `X-Request-Id` is written by the gateway itself (the client's value when supplied, otherwise a generated UUID), so forwarding it either leaked the gateway's internal correlation id to a third party or let a client inject an unvalidated value; note it is not the same id the gateway stores in usage logs.
- **The Codex WebSocket face now forwards no client headers at all**: previously it relayed every client header surviving the shared deny list except a seven-item disguise list, so custom headers such as `X-Opencode-Session` and `X-Client-Version` did reach upstream, and a client `User-Agent` or `Originator` could override the disguised identity. That face now sends only what the SDK writes for it — disguise identity, beta value, session family and credential — so upstream always sees the SDK's disguise values and no client header can influence the handshake it sees. The other three faces are unchanged.
