...
- **Routing observability is paginated and folded, and two list defaults change (behaviour change, deliberate)**: the three read endpoints behind the console's routing section (`GET /api/admin/routing/plan|flow|frontier`) now answer one server-clamped page instead of the whole collection — `offset`/`limit` default `0`/`20` with a `200` cap on all three, plus a second per-route candidate axis (`candidates_offset`/`candidates_limit`) on `/routing/plan`, and a per-layer top-N account fold (`accounts`, default 20) on `/routing/flow`. Callers that pass no parameters therefore see less than before on two endpoints: `/routing/frontier` returns 20 rows where it returned 200, and `/routing/plan` returns the first 20 routes where it returned the entire plan. Polling scripts that omit `limit` should pass one explicitly; `/routing/plan` also gained `search` (case-insensitive substring over model / format / operation tag / group id) and `route` (exact, takes precedence over `search`) so a single route can be fetched directly. Paging never changes meaning: `total_edges`, `total_chains`, `stale_chains`, `total_candidates`, `total_routes` and the Sankey graph are computed over the complete set regardless of the current page, and folding only merges accounts — each layer's chain count is preserved, so the first-dispatch/terminal conservation check still holds. The old-generation badge is no longer derived in the browser: the server returns `stale_chains` and `total_chains` as integers and the console renders the ratio, so the badge no longer flickers while paging.
