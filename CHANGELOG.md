...
- **The Codex WebSocket face now forwards no client headers at all**: previously it relayed every client header surviving the shared deny list except a seven-item disguise list, so custom headers such as `X-Opencode-Session` and `X-Client-Version` did reach upstream. That face now sends only what the SDK writes for it — disguise identity, beta value, session family and credential — so no client header can influence the handshake upstream sees. The other three faces are unchanged.
