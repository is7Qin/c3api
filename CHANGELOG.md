...
- **Client-IP and forwarding-path headers no longer reach upstream (behaviour change)**: `X-Forwarded-For`, `X-Real-IP`, `CF-Connecting-IP`, `True-Client-IP`, `Forwarded`, `X-Forwarded-Host`, `X-Forwarded-Proto` and `Via` are now stripped on every face. These describe the client's own connection — who the peer was, and which host, scheme and intermediaries it came through — while upstream sees the gateway as its peer and the gateway rewrites the target URL. The gateway also does not append its own observation the way a conventional reverse proxy does, so relaying them passed a client-controlled value upstream in a shape that looks proxy-generated. This is deliberately **not** a security boundary: the list is a deny list, so other keys carrying the same kind of information (for example `X-Client-IP`, or any custom header) still reach upstream, and only the well-known carriers are removed.
