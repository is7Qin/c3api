...
- **The partition-retention worker now reports whether it is actually keeping the routing tables bounded**: `GET /api/admin/ops/workers` gains `partition_count` and `oldest_partition_unix_ms` on the `retention` worker, and that worker now warns when the oldest routing partition is older than the configured observation-retention cutoff. The routing observation tables stay bounded only while this worker runs; if it stops or its `DROP` keeps failing, they grow without limit and nothing else notices, so these two values are the signal. In a healthy system the oldest partition is never older than the cutoff, which is what makes the warning meaningful rather than noise.
