...
| **quality.SyncWorker** | "quality-sync"（`internal/quality/sync.go:229`） | 串行 loop | 质量/flow 脏行 UPSERT 进实例分钟表（`routing_quality_instance_minute` / `routing_flow_instance_minute`） | Close 取消 |
| **quality.FlowOwner / RollupWorker** | "quality-flow-owner"（`internal/quality/fold_owner.go:112`）/ "routing-rollup"（`internal/quality/rollup.go:97`） | loop / 脏分钟消费 | 各实例脏分钟合并进全局 rollup 表，quality/flow 两道各自推进 `routing_rollup_watermark` | Close 取消 |
