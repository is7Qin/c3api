...
| `internal/sdkbridge` | codex-sdk 适配层（生图/Responses/Stream/WS/Search/Dial + 凭据派生 clientFor + 失效回调 + OAuth 轮转回写） | proxy |
...
- **配额**：`quotaExhausted`（`internal/proxy/gate.go:267-319`）本地预算两原子读；耗尽才触发 DB 复核认领（`internal/proxy/gate.go:320-374`，慢路径单飞 + 10s 失败退避）；复核公式 `budget = consumed + ceil(remaining_eff/N)`（收敛修正，防复核无限续额）。
...
- **流式透传**：aiclient 流式入口经 `pkg/sserelay` 字节级 relay + Observer 旁路提取 usage；批次能力：EOF 末帧 flush（`relay.go:217-230`，无末尾空行上游丢 completed 帧 → cost=0 修复）、deadline watcher（`relay.go:353-376` + `middleware.go:154-156`）、normalize 错误分类（`relay.go:269-277` 三态可分）、relayBufio 池（`relay.go:86-98`）、按需武装 flush timer（`relay.go:307-323`）；WS 1:1 透传（`internal/proxy/caller_responses_ws.go:272`，心跳 :352）。
...
3. **开关开启 = bytes.Contains 预筛零解析**：图像 tool 剥离 `internal/proxy/strip_image.go` 预筛（无 "image" 子串零解析，压测 ~1.5% QPS 差异）；转换走 gjson 预筛 + Raw 零拷贝（`internal/protoconv/jsraw.go:1-9`）。
...
- `internal/sdkbridge`（`internal/sdkbridge/codex.go:16`）：**codex-sdk 唯一适配层**——GenerateImage/GenerateImageStream/Responses/StreamResponses/Search/Dial + 凭据派生 clientFor + 失效回调 + 轮转回写 WriteOAuthRotation；codex 非流式超时各自包 ctx WithTimeout（`codex_responses_http.go:92`、`caller_images_codex.go:111`——HTTPClient.Timeout 不可用，流式/非流式四方法共享，`forward.go:36` 注释）。
...
- `pkg/sserelay`：字节级 SSE relay——增量读帧原样转发 + 自适应批量 Flush + Observer 旁路（仅 usage 提取，不参与转发决策；`pkg/sserelay/relay.go:5-8`）+ EOF 末帧 flush + deadline watcher + normalize 错误分类 + relayBufio 池 + **Mapper 挂载**（转换，`relay.go:57-62`，Observer 仍见原始帧）。
...
- 缺名帧处理（教训）：无 `event:` 名帧从 data 的 `type` 字段推断（`internal/protoconv/protoconv.go:167-175` `inferEventName`，Map 内分派 :116-125），无法推断原样透传。
...
- 关键索引（`internal/repository/partition.go:141-148,178-182,233-236`）：usage_logs `(created_at)` + `(group_id/account_id/user_id/key_id, created_at)` + **`usagelog_request_id_created_at` 唯一索引**（`partition.go:147`，幂等键）；err_logs `(created_at)` + `(group_id/user_id, created_at)`；usage_stats 唯一索引 `(bucket_time, group_id, account_id, template_id, user_id, model, is_error)`（即 Upsert 冲突目标）+ **`usagestat_user_id_bucket_time` 复合索引**（`partition.go:235`——bucket_time 冗余独立索引已删后的覆盖索引）。
...
**main 现状（已合并）**：快照注册表 `snapshot.Registry`（`internal/snapshot/snapshot.go`：ReloadAll 启动就绪 :126-133、Reload 空 scopes 前置 return :139-142、scope 精确分发 :143-159）只持有 Name/Scope/LastReload 元数据，**不接管模块 ticker、不缓存快照数据、不进请求热路径**（用户拍板 2026-08-11，`snapshot.go:9-19` 包注释）。快照数据形态与周期刷新仍各模块自管（atomic.Pointer 整表换入或 RWMutex + 锁内换 map）：
...
| pricing（模型价格**三线**：pricing + image_price + function_price） | `internal/service/pricing.go:35-71` + `image_pricing.go:23` + `function_pricing.go:23` | ReloadPricing 三线（sync 成功/管理端改价后，`main.go:369-373`）+ NOTIFY Pricing → dispatcher 同步 ReloadPricingCtx（settings 同款，跨实例失效） |
...
- **启动双刷**：main 侧 `snapReg.ReloadAll` 统一首刷（`main.go:443-456`，全成功置位 bootLoaded）→ dispatcher 首连 FullRefresh CAS 消费后跳过五路仅补 ReloadSettings（`dispatcher.go:41-49,136-155`）；部分失败不置位 → 首连仍全量。
...
- **NOTIFY 跨实例**（§9 全链）：实例 ID = **hostname-pid-nonce**（crypto/rand 6B 随机，`cmd/server/main.go:118-130` 装配 + `instanceSrc` :526-536）——修复：容器化多实例同 hostname、pid namespace 各自 pid 1 → 纯 hostname-pid 互相碰撞 → 互把对方 NOTIFY 当自播跳过 → 失效静默全灭；自播判等 = 全串相等 `ch.Src == l.cfg.Src`（`listener.go:250-251` 注释）。
...
- **分区 DROP 幂等**（`internal/repository/partition.go:301-307,358,421-426`）：IF NOT EXISTS / IF EXISTS + 撞名 42P07/42710/23505（`isDuplicateObject` :301-307）+ **42P01 stale-DROP 窗口**（`isMissingObject` :318-324、`isBootstrapRaceError` :329-331——并发实例基于过期"未分区"判定 DROP 误删对方刚建表，由最后执行 DROP 的实例补建收敛，已接受）；retention DROP 需 ACCESS EXCLUSIVE 锁与在途插入串行（`internal/usage/retention.go:60-64` 注记）。
- **失效分发兜底面**：auth-sync 60s 兜底新增 per-attempt 30s 超时（`auth_sync.go:21-26,80-94`）+ 托管 goroutine（:37-40,57,70）；规则重载 Background ctx 不随请求取消（`invalidate.go:347-354`）。
...
| `[db]` | repository.OpenPG | dsn/max_conns（20 = billing 8 + stats 8 worker + 余量）；**OpenPG 自动补丁**：lock_timeout=5s 会话级 + 计费扣费 per-query 10s 超时 + MaxConnLifetime=30m 滚动轮换——DSN 无需手工配置，用户 DSN 显式同名参数时尊重不覆盖（`config.go:56-61` + `main.go:134` 注释） |
...
9. **锁顺序一致化防死锁**——批量 upsert 批内按 model 排序 + 40P01 重试。为什么：多实例并发同批 model 取锁顺序交错 → deadlock detected（压测启动期偶发）；排序消除主因，重试兜底残余。来源：`internal/repository/pricing_repo.go:62-66`（sort.SliceStable）+ `internal/repository/litellm_upsert.go:19,28,61,92-96`（litellmExecBatchWithRetry / isDeadlock）。
...
- **风暴教训**（压测 2026-08-11 修复链，`internal/repository/pricing_repo.go`/`internal/billing/flusher.go`/`internal/repository/partition.go` 注释 + `docs/superpowers/plans/2026-08-11-errlog-task.md`）：
  - **分区漂移**：price 快照列/ttft_ms 合入后旧分区表缺列 → 新二进制连旧库 INSERT 42703 全停；当时修复 = bootstrap 幂等补列 ALTER（`partition.go:83-95,357-362`）——**该机制已删（2026-08-15）**，现以**全新建库**消除存量漂移面（无补列路径）。
  - **巨批**：单用户积压 1M+ 行 → 单事务 2000+ 分片串行 8 分钟（xact_age 08:02 实证）+ 堆涨 4.6GB；修复 = 单用户拆事务 ≤10k 行/事务（`internal/billing/flusher.go:34-39`）。
  - **缺名帧**：非规范上游缺 `event:` 行 → 流式转换断流；修复 = data `type` 字段推断事件名（`internal/protoconv/protoconv.go:167-175`）。
  - **拒绝风暴**：单 key 限流 161k req/s → 60s 冲至 9.8M pending 行 / RSS 7.5GB；修复 = 拒绝行不入 usage_logs 明细 + err_logs 有界队列采样（`internal/proxy/forward.go:347` recordRejected）+ 积压续传循环 `backlogDrainBudget`（`internal/billing/flusher.go:41-51`）。
  - **停机截断**：复测在途批次 Background ctx 令停机拖至分钟级；修复 = baseCtx 可取消 + Close 预算约束截断 Warn（`internal/billing/flusher.go:95-99,228-283`）。
