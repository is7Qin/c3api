...
| usage.RetentionWorker | "retention"（`internal/usage/retention.go:87`） | ticker 1h | 三张用量表独立 cutoff 各自 DROP + 预建当日/明日，**并对路由两张事实表按 `routing.observation_retention_days` 同一 cutoff DROP 日分区 + 清理 `routing_flow_snapshot_state`**；逐表错误隔离（`internal/usage/retention.go:147-294`）；**另含 redemption_uses 90 天 TTL 有界批删**（≤5000 行/轮，普通表无分区可 DROP，`retention.go:44,50,234` + `partition.go:587`）；启动即巡检 runOnce（:123）；观测面 lastPatrol/lastDrop* + **oldest_partition_unix_ms/partition_count（聚合告警）+ quality/flow 分表计数与最老（故障定位）+ last_dropped_routing_quality/flow_partitions（DROP 进度）+ snapshot_state_rows（交接状态行数）+ undated_partition_count（无名分区数）+ partition_stats_stale（过期值标记）+ routing_observation_retention_days（cutoff 解释口径）；保留期兜底：路由最老分区早于 cutoff ⇒ Warn，`retention.go:97-105,271-291`）** | 无排空需求（DROP/预建/批删均幂等），Close 直接 nil（`internal/usage/retention.go:296`） |
