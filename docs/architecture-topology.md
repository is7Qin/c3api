...
| **quality.SyncWorker** | "quality-sync"（`internal/quality/sync.go:229`） | 串行 loop | 质量行 UPSERT 进实例分钟表（`routing_quality_instance_minute`）；flow 快照**直写合并层**（`routing_flow_rollup`，按 `instance_src` 分片、整分片替换，无下游重算） | Close 取消 |
