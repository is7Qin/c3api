...
func TestRed_CursorGenerationFencingRedis(t *testing.T) {
	...
	w := NewSyncWorker(rec, rdb, pg, SyncConfig{InstanceSrc: "red-gen-redis", BatchSize: 10}, nil)
...
func TestRed_CursorGenerationFencingPG(t *testing.T) {
	...
	require.NoError(t, err)
	base := time.Date(2026, 8, 29, 12, 0, 0, 0, time.UTC)
	w := NewSyncWorker(rec, rdb, pg, SyncConfig{InstanceSrc: "red-gen-pg", BatchSize: 10}, nil)
	w.SetClock(func() time.Time { return base })
...
func TestRed_CloseBoundedDrainContinuesUntilEmpty(t *testing.T) {
	...
	w := NewSyncWorker(rec, rdb, pg, SyncConfig{InstanceSrc: "red-close-drain", BatchSize: 10}, nil)
...
func TestRed_CloseReturnsRemainingWorkError(t *testing.T) {
	...
	require.NoError(t, err)
	fixed := time.Date(2026, 8, 29, 12, 0, 0, 0, time.UTC)
	w := NewSyncWorker(rec, rdb, pg, SyncConfig{InstanceSrc: "red-close-remaining", BatchSize: 10}, nil)
	w.SetClock(func() time.Time { return fixed })
...
func TestRed_BisectDropsOnlyTypedAndRefillsTransient(t *testing.T) {
	...
	require.NoError(t, err)
	fixed := time.Date(2026, 8, 29, 12, 0, 0, 0, time.UTC)
	w := NewSyncWorker(rec, rdb, pg, SyncConfig{InstanceSrc: "red-bisect-typed", BatchSize: 10}, nil)
	w.SetClock(func() time.Time { return fixed })
	...
	w2 := NewSyncWorker(rec2, rdb, pg2, SyncConfig{InstanceSrc: "red-bisect-poison", BatchSize: 10}, nil)
...
func TestRed_BisectTransientInChunkMustRefill(t *testing.T) {
	...
	fixed := time.Date(2026, 8, 29, 12, 0, 0, 0, time.UTC)
	w := NewSyncWorker(rec, rdb, pg, SyncConfig{InstanceSrc: "red-bisect-chunk-transient", BatchSize: 10}, nil)
	w.SetClock(func() time.Time { return fixed })
