...
// TestMergeFlowRowsBounded_InputIndependence asserts the bounded merge's
// ownership contract directly: result rows are fresh clones with their own
// PreviousAccountID pointers, inputs are never mutated, the result capacity
// stays bounded by rowCap, and only incoming rows past the cap count as
// drops.
func TestMergeFlowRowsBounded_InputIndependence(t *testing.T) {
	...
	require.LessOrEqual(t, cap(merged), 8, "merged capacity stays bounded by rowCap")
	require.Equal(t, int64(10), merged[0].AccountID, "incoming leads the merged order")
	incoming[0].ChainCount = 999
	existing[0].ChainCount = 999
	prevIn = 42
	prevEx = 43
	require.Equal(t, int64(1), merged[0].ChainCount, "result must not alias input row structs")
	require.Equal(t, int64(1), merged[1].ChainCount)
	require.Equal(t, int64(1), *merged[0].PreviousAccountID, "result deep-copies PreviousAccountID")
	...
	require.Equal(t, int64(1), dropped, "incoming rows past the cap are counted as dropped")
	require.Len(t, kept, 2, "incoming leads; excess existing rows are displaced without a drop count")
	require.Equal(t, int64(40), kept[0].AccountID)
	require.Equal(t, int64(50), kept[1].AccountID)
}

// TestFlowOwner_CloseIdempotentAndJoins pins the worker lifecycle contract:
// Close is safe before Start and idempotent after; a started loop is joined
// (loopDone closed) before Close returns; no restart after close.
