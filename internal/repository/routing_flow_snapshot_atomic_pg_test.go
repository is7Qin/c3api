...
func TestRoutingFlowSnapshotAtomicRollbackPG(t *testing.T) {
	...
	fp1 := mustFPVal(t, 3, 10, credential.TypeAPIKey, "https://api.openai.com", "sk-y", "", "", "", false, "", "", "", "")
	fp2 := mustFPVal(t, 4, 10, credential.TypeAPIKey, "https://api.openai.com", "sk-z", "", "", "", false, "", "", "", "")
	src := "src-atomic"

	// Seed seq1 full rows.
	seq1 := []repository.RoutingFlowRow{
		{IdentityVersion: 1, RouteClassID: rc, TerminalMinute: now, Ordinal: 1, Lane: "primary", AccountID: 3, TransitionReason: "initial", Outcome: "success", IsTerminal: true, Generation: 1, CandidateFingerprint: fp1, InstanceSrc: src, ChainCount: 1},
		{IdentityVersion: 1, RouteClassID: rc, TerminalMinute: now, Ordinal: 2, Lane: "primary", AccountID: 4, PreviousAccountID: ptrInt64(3), PreviousOutcome: "success", TransitionReason: "retry", Outcome: "success", IsTerminal: true, Generation: 1, CandidateFingerprint: fp2, InstanceSrc: src, ChainCount: 1},
	}
	...
	_, err = pool.Exec(ctx, `DROP TRIGGER IF EXISTS tmp_flow_fail ON routing_flow_instance_minute`)
	require.NoError(t, err)
	_, err = pool.Exec(ctx, `CREATE TRIGGER tmp_flow_fail BEFORE INSERT ON routing_flow_instance_minute FOR EACH ROW WHEN (NEW.chain_count = 777) EXECUTE FUNCTION tmp_flow_fail_sentinel()`)
	require.NoError(t, err)
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DROP TRIGGER IF EXISTS tmp_flow_fail ON routing_flow_instance_minute`)
	...
	seq2 := []repository.RoutingFlowRow{
		{IdentityVersion: 1, RouteClassID: rc, TerminalMinute: now, Ordinal: 1, Lane: "explore", AccountID: 5, TransitionReason: "initial", Outcome: "success", IsTerminal: true, Generation: 2, CandidateFingerprint: fp1, InstanceSrc: src, ChainCount: 2},
		{IdentityVersion: 1, RouteClassID: rc, TerminalMinute: now, Ordinal: 2, Lane: "explore", AccountID: 6, TransitionReason: "retry", Outcome: "success", IsTerminal: true, Generation: 2, CandidateFingerprint: fp2, InstanceSrc: src, ChainCount: 777},
	}
	...
	var cnt int64
	require.NoError(t, pool.QueryRow(ctx, `SELECT COUNT(*) FROM routing_flow_instance_minute WHERE instance_src=$1 AND terminal_minute=$2`, src, now).Scan(&cnt))
	require.Equal(t, int64(2), cnt, "failed replacement must leave old rows untouched")
	var accounts []int64
	rows, err := pool.Query(ctx, `SELECT account_id FROM routing_flow_instance_minute WHERE instance_src=$1 AND terminal_minute=$2 ORDER BY ordinal`, src, now)
	require.NoError(t, err)
	for rows.Next() {
		var a int64
	...
	_, err = pool.Exec(ctx, `DROP TRIGGER tmp_flow_fail ON routing_flow_instance_minute`)
	require.NoError(t, err)
	seq2[1].ChainCount = 3
