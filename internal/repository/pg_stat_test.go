...
func TestPGStatsRawCostRoundtrip(t *testing.T) {
	...
	sum, err := repos.Stats.SummarizeStatsCube(ctx, h, h.Add(time.Hour), 0, time.UTC)
	require.NoError(t, err)
	require.Equal(t, int64(840), sum.RawCost, "summary SUM(raw_cost)（840+0）")
	days, err := repos.Stats.ScanStatsDaysCube(ctx, h, h.Add(time.Hour), 0, time.UTC)
	require.NoError(t, err)
...
func TestPGStatsSummarizeTTFT(t *testing.T) {
	...
	sum, err := repos.Stats.SummarizeStatsCube(ctx, bucket, bucket.Add(time.Hour), 0, time.UTC)
	require.NoError(t, err)
...
func TestPGStatsSummarizeNoData(t *testing.T) {
	...
	trend, err := repos.Stats.ScanStatsDaysCube(ctx, bucket, bucket.Add(24*time.Hour), 0, time.UTC)
	require.NoError(t, err)
