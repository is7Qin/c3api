...
func TestPGStatsTrendPushdown(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		got, err := repos.Stats.StatsTrendCube(ctx, h, h.Add(2*time.Hour), tc.unit, tc.groupID, tc.model, time.UTC)
	...
	_, err := repos.Stats.StatsTrendCube(ctx, h, h.Add(time.Hour), "week", 0, "", time.UTC)
	require.Error(t, err)
}
...
func TestPGStatsEntityTrendAndModelSplit(t *testing.T) {
	...
	got, err := repos.Stats.StatsEntityTrendCube(ctx, from, to, "hour", "account", 7, "", time.UTC)
	require.NoError(t, err)
	require.Len(t, got, 2)
	...
	got, err = repos.Stats.StatsEntityTrendCube(ctx, from, to, "hour", "account", 7, "m1", time.UTC)
	require.NoError(t, err)
	require.Len(t, got, 2)
	...
	got, err = repos.Stats.StatsEntityTrendCube(ctx, day, day.Add(24*time.Hour), "day", "account", 7, "", time.UTC)
	require.NoError(t, err)
	require.Len(t, got, 1)
	...
	_, err = repos.Stats.StatsEntityTrendCube(ctx, from, to, "month", "account", 7, "", time.UTC)
	require.Error(t, err)
	_, err = repos.Stats.StatsEntityTrendCube(ctx, from, to, "hour", "template", 7, "", time.UTC)
	require.Error(t, err)
}
