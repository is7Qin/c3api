...
func TestConvertedMappingREST(t *testing.T) {
	...
	for _, tc := range dirs {
		...
		t.Run(tc.name+"/implicit_rewrites_to_client", func(t *testing.T) {
			...
			accs := map[int64][]*domain.Account{10: {{ID: 1, TemplateID: 1, Template: tpl, UpstreamKey: "sk-upstream", Enabled: true, LifecycleRevision: 1, IdentityRevision: 1, MaxConcurrency: 4}}}
