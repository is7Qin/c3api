...
// --- codex 独立 relay 变体（codex-oauth/codex-pat 类型 resp-ws 接线） ---
...
// *websocket.Conn 不动。
//
// 凭据双线分工：relay 线 = 快照（sel.Ext → AccountExt）→
...
// handleCodexDialError codex WS 拨号失败分类与收尾（返回
...
//     **该请求不转移**——finish（code 0 ErrNetwork）+ 错误帧收尾
