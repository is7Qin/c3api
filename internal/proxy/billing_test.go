...
func TestProxyFinishBillingDisabledSkipsQuotaDeduction(t *testing.T) {
	...
	// When
	p.finish(nil, &domain.UsageLog{KeyID: 1, Cost: 130})
...
func newTestProxyBillingKeys(t *testing.T, keys map[string]domain.KeyMeta, accs map[int64][]*domain.Account, bal *billing.Balances, logs usage.LogInserter, qws ...usage.QuotaWriter) *Proxy {
	t.Helper()
	cfg := Config{
