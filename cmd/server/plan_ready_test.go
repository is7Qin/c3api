...
func newReadyScheduler(t *testing.T) *scheduler.Scheduler {
	t.Helper()
	re := rule.New(rule.Config{}, readyRuleStore{}, nil, nil, nil)
	return scheduler.New(scheduler.Config{DefaultMaxConcurrency: 4, SyncInterval: time.Hour}, readyLoader{}, re, nil, nil, nil, nil)
}
